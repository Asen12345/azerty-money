<h2>Оформить заказ</h2>
						...
						<p class="leave_a_comment-b" name="sum_game" style="display:none;">Цена за 1кк: <span></span></p></br>
						<div class="clr"><span> </span></div>
...
<script>
							$(document).ready(function(){
								$('input[id=radio1]').change(function(){
									...
									$('span[name=text1]').text('Передача через совместный рейд');
									$('span[name=text2]').text('');	
								});	 	 
								$('input[id=radio3]').change(function(){
									$('span[name=text2]').text('Заказ может доставляться по частям в течении указанного срока доставки. Поэтому не переживайте, если первой почтой у вас придет меньше товара, чем указано в заказе ');
									$('span[name=text1]').text('');
								});		 
							});
						</script>
                        ...
                        <p class="p-delivery_method">
                            ...
                            <label class="radio-delivery_method">
                                <input name="Способ доставки" type="radio" id="radio2" value="Рейд"><span></span>
                            </label>
                            <label for="radio2">Рейд</label><br>
