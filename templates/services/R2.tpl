<h2>Оформить заказ</h2>
						...
						<p class="leave_a_comment-b" name="sum_game" style="display:none;">Цена за 1 кк серебра: <span></span></p></br>
						<div class="clr"><span> </span></div>
...
<script>
		$(document).ready(function(){
			...
			$('input[id=radio2]').change(function(){
				$('span[name=text1]').text('Передача через Лавку. Выставите на аукцион любой предмет и укажите его название в Комментарии');
				$('span[name=text2]').text('');
				$('span[name=text3]').text('');
			});
		});
	...
	<p class="p-delivery_method">
		...
		<label class="radio-delivery_method">
			<input name="Способ доставки" type="radio" id="radio2" value="Передача через Лавку"><span></span>
		</label>
		<label for="radio2">Передача через Лавку </label><br>
