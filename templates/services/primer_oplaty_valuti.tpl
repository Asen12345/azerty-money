<!--По содержанию тегов <p> понятно что есть на странице-->
					   <h2>Оформить заказ</h2>
                        <p class="leave_a_comment-p">Выберите сервер</p>
                        <select name="server" class="gold_game_selection">
							<option selected value="">---------------------------------------</option>
						{foreach key=key item=item from=$servers}
							<option value="{$item.id}">{$item.caption}</option>
						{/foreach}
						<!--В скрипте ниже выполняется расчет цен за единицу валюты, если единица валюты 1000, то оставить скрипт таким же,если 1, то заменить return  1000 на 1 -->
                        </select> 
						<script>
                        ...
                        <p class="leave_a_comment-p">Укажите сторону</p>
                        <input class="gold_input_name" name="Укажите имя персонажа">
                        <p class="leave_a_comment-p">Укажите имя персонажа</p>
                        ...
                        <p class="leave_a_comment-p">Выберите способ оплаты</p>
                        <p class="p-buy">Я покупаю <input class="input-buy_paying" name="calc1"></p>
						...
						<!-- Далее идет добавление описания способов доставки -->
						<script>
							$(document).ready(function(){
								...
								$('input[id=radio2]').change(function(){
									$('span[name=text1]').text('Встреча в игре (Оргриммар, Штормград)»: «Встреча в игре в Оргриммаре (орда) или Штормграде (альянс)');
									$('span[name=text2]').text('');
									$('span[name=text3]').text('');
								});	  
								...
								$('input[id=radio4]').change(function(){
									$('span[name=text1]').text('');
									$('span[name=text2]').text('');
									$('span[name=text3]').text('Выставьте на аукцион любой зеленый/синий/эпический предмет с выкупом равным сумме заказа +6%. Но сумма выкупа не должна превышать 25 000 - 30 000 золота. Если у вас заказ на большую сумму, поставьте несколько лотов. Название предмета укажите в комментарии ниже');
								});	  
							});
						</script>
                        <p class="leave_a_comment-p">Способ доставки</p>
