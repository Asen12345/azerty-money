  ...
  $results = ORM::for_table('games')->where('id', $person['id'])->find_one();
   ...
   $results->save();
        ...
        public function elementServicesNew()
        {
            ...
            if ($handle = opendir('data/img-back'))
            {
                while (false !== ($entry = readdir($handle)))
                {
                    if ($entry != "." && $entry != "..")
                    {
                        $photos1[] = '/data/img-back/' . $entry;

                        arsort($photos1, SORT_NUMERIC);
                    }
                }
                closedir($handle);
            }
            view::assign("servicesDataPhotos1", $photos1);
           ...
           $_SESSION['par'] = $par;
            ...
            //if( $_FILES['testimg']['name'][0] != ""){ $update->back =     $_FILES['testimg']['name'][0];}// $_POST['testimg']
             //  if($_FILES['testimg1']['name'][0] != ""){    $update->front =   $_FILES['testimg1']['name'][0];} //$_POST['testimg1']
                ...
                $update->front = $_POST['services_photo'];

                $update->back = $_POST['services_photo1'];
        ...
        public function editItem()
        {
            ...
            foreach ($data['services'] as $key => $value)
            {
                $row = ORM::for_table('games')->order_by_asc('caption')->where('id', $value['gid'])->find_one();
                $data['services'][$key]['game_caption'] =   $row->caption;
            ...
            foreach ($data['services'] as $key => $value)
            {
               $row = ORM::for_table('games')->where('id', $value['gid'])->find_one();
                ...
                $rowt = ORM::for_table('games')->where('caption', $row->caption)->find_one();
             
$tesv = ORM::for_table('services')->order_by_asc('caption')->where('id', $rowt->id)->where('type', 1)->find_one();

//print_r($tesv ->caption . " ");


$data['services'][$key]['gamin1'] =  array(1, 2, 3, 4, 5) ; //$tesv->caption;
            ...
            view::assign('id', $data['services'][$key]['gamin1']);
            view::assign("servicesItemData", $data);
